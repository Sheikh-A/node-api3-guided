...
router.use((req,res,next) => {
  ...
  console.log('inside hubs router!');
  next();
});
  ...
  Hubs.findById(id)
    .then(hub => {
      if(hub) {
        ...
        res.status(404).json({message: "hub id not ofund"});
      }
    })
    .catch(err => {
...
function requiredBody(req,res,next) {
  const body = req.body;
  !body || body === {} ? //IF ( statement )
    res.status(400).json({ message: "Please include request body" }) //do this
  : next(); //: else do this 
}
